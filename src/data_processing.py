...
class DataProcessor:
    ...
    def augment_data(self, static_features, time_series_data, target, factor=2):
        ...
        for i in range(1, factor):
            idx = n_samples * i
            # 針對不同特徵使用不同量級的噪聲
            static_noise = np.random.normal(0, 0.03, static_features.shape)
            ts_noise = np.random.normal(0, 0.03, time_series_data.shape)
            
            # 目標變數的噪聲應該很小，保持數據的物理意義
            ...
            aug_target[idx:idx+n_samples] = np.maximum(0.001, augmented_target)
